...
var MainCtrl = function($scope, github, $interval, $log, $anchorScroll, $location){
    var OnUserComplete = function(data){   /*return when data come succefully from api*/
        $scope.user = data;           /*store response data in user scope*/
            
        github.getRepos($scope.user).then(onRepos,on_errors_repos);        
    };
     
        var onRepos = function(data){
            $scope.repos = data;   /*on succesive get data of repositary*/
   ...
   $scope.search = function(username){
        ...
        github.getUser(username).then(OnUserComplete,on_errors_repos);
